...
export default function exponentiate(x: number): NumberCallback
export default function exponentiate(x: number, ...y: number[]): number
export default function exponentiate(...x: number[]): number | NumberCallback {
  return length(x) === 1
